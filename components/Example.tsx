...
export default function Example({ examples }: any) {
  ...
  return (
    <div className="border-2 border-l-8 mx-3 p-5 rounded-lg border-[#264653]">
      <ul>
        {status === 'success' ? Examples.slice(0,count).map((example,index) => (
          <div key={index} className='flex text-gray-600 font-medium text-lg mb-3'>
            <span className="mr-2">{index+1}.</span>
            <li>{example !== undefined && example}</li>
          </div>
        )) : <div >
          <Skeleton height={30} count={6} className='mb-2 w-[250px] sm:w-[280px]'/>
          <Skeleton width={150} height={30} />
        </div>
      }
      </ul>
      <button className="flex mx-auto text-gray-500 font-medium text-lg" onClick={handelClick}>More</button>
    </div>
  );
}
